...
Hci.prototype.createLeConn = function(address, addressType) {
  ...
  const KNEX_CONN_INTERVAL = 100;
  const KNEX_CON_SUP_TIMEOUT = 3000;
  const KNEX_CONN_EVENT_LENGTH = (12 * 1.25)

  cmd.writeUInt16LE(Math.floor(KNEX_CONN_INTERVAL / 1.25), 17); // min interval
  cmd.writeUInt16LE(Math.floor(KNEX_CONN_INTERVAL / 1.25), 19); // max interval
  cmd.writeUInt16LE(0x0000, 21); // latency
  cmd.writeUInt16LE(Math.floor(KNEX_CON_SUP_TIMEOUT / 10), 23); // supervision timeout
  cmd.writeUInt16LE(Math.floor(KNEX_CONN_EVENT_LENGTH / 1.25), 25); // min ce length
  cmd.writeUInt16LE(Math.floor(KNEX_CONN_EVENT_LENGTH / 1.25), 27); // max ce length
